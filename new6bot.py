...
ADMIN_ID = [679030634, 927878071, 127646663, 187831178]

def is_admin(user_id):
    return user_id in ADMIN_ID

API_TOKEN = '6708762418:AAFEgHko1kjBUrbO77Xd_nUjVC9SuTi1x6A'
...
async def show_main_menu(chat_id):
    ...
    photo_path = 'fotoval/Frame 28.jpg'
...
async def invite_friend(call: types.CallbackQuery):
    ...
    invite_link = f'https://t.me/tutor_100ballniy_socv_bot?start={call.from_user.id}'
    await call.message.answer(
        'Раздел с приглашениями на доработке! Зови друзей, чтобы они тоже получили крутые призы в конце месяца 🤓\n\n'
        f'Вот твоя ссылка: {invite_link}\n\n'
        'P. S. Менеджеру придёт сообщение о вступлении твоего друга. В течение дня он начислит тебе баллы ❤️'
    )
...
async def join_channels_and_chats(call: types.CallbackQuery):
    ...
    join_button_url = "https://t.me/addlist/JfVJp4jxQjBkMDMy "
